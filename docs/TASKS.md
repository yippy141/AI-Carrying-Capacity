...
- [x] Complete the bounded issue #35 P1 correction pass: fix the PRC final-law locator, resolve the HEAT-ML/ITER/NSTX-U locators, add targeted IFMIF-DONES materials evidence, correct licensing S2/S4 support, and type legal status precisely without reopening S1-S5.
- [x] Complete issue #37 source-record promotion after merged PR #36: 44 staged records reviewed and promoted with stable IDs; zero new deferrals; two excluded candidates remain out. Add the decision ledger, promotion note, and source-only integrity guards without approving claims/profiles.
- [x] Verify issue #37 source-promotion PR #38 merged before fusion domain review, including exact head `3c8987175f6975347cc01a768c21d3386ff27cd6` in main (merge `a85267ce3a0c6ec598554e45b9bc4a82a74f9e95`).
- [ ] Preserve native-language/specialist quotation and translated-wording blocks; refresh the STEP living page successfully before public/load-bearing target use (promotion-time TLS certificate failure).
- [x] Complete issue #39's AI-assisted technical/domain review of all 18 frozen fusion profiles and 90 separate S cells using the reviewed canonical evidence; record `model_domain_synthesis`, preserve all nine empirical gaps, and produce the five review outputs plus validators/tests. No S implementation, human expert sign-off, canonical approval or WP2 work.
- [x] Complete issue #39's one substantive PM review and bounded correction pass: accept 88 recommendations unchanged, correct construction and tritium/fuel-cycle S2, retain nine gaps, and update the 28 owner exceptions without reopening M1.5.
- [x] Add `docs/PROJECT_STATE.md` as a derived execution checkpoint and synchronize the roadmap, PM status, agent brief, and README pointer without changing the authority order or frozen method.
- [ ] Pursue the 19 exact named-specialist questions through six coordinated outreach packages. They block approved/canonical and unqualified public use, while `EXPERT-CODED · DRAFT` public-pilot rows, private use, and staged WP2 construction remain permitted with all method caveats visible. Scope six technical-variant cells and two licensing-jurisdiction cells without changing shared S values by country.
- [ ] Preserve and revisit the nine load-bearing fusion empirical gaps; model recommendations do not supply missing qualification, fuel-cycle, blanket, commissioning, reliability, licensing or grid outcomes.
- [x] Complete targeted S5 adjudication for exactly 19 affected rows using the clarified boundary convention, the seven owner-routed S5 decisions, and the preserved 12-row audit trail: 12 provisional points, six preserved ranges, and one preserved disagreement; do not reopen S1-S4.
- [x] Define the human/domain-review plan by retaining six fusion packages and adding one software and one manufacturing package; all eight block canonical approval, not private use, staged WP2, or visibly labelled draft display.
...
- [ ] Verify the draft PR's hosted CI result after pushing the release branch.
