...
- [ ] Review the issue #39 draft domain-review package in one substantive PM pass and one correction pass before merging; resolve its 27 owner exceptions without repeating all 90 routine reviews.
- [ ] Obtain named specialist input for the 19 exact questions in `domain-review/fusion_human_expert_queue_v1.csv` before the affected cells' public/canonical approval; scope six technical-variant cells and two licensing-jurisdiction cells without changing shared S values by country.
